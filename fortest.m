...
RSB_NUM = 3;%length(EV_RSB_dir); % calculate length of RSB data
...
%hdfread(..., 'Index', {start,stride,edge})
    ...
    EV_1000_RefSB(EV_1000_RefSB > 32767) = NaN;
    EV_1000_RefSB = double(EV_1000_RefSB);
...
for idx = 1:RSB_NUM
    ...
    Longitude_GEO(Longitude == -999) = NaN;
    Longitude_GEO = double(Longitude_GEO);
toc
...
for idx = 1:RSB_NUM
    ...
    Latitude_GEO(Latitude_GEO == -999) = NaN;
    Latitude_GEO = double(Latitude_GEO);
...
for idx = 1:RSB_NUM
    writematrix([Longitude_GEO(:,1,idx),Latitude_GEO(:,1,idx),EV_1000_RefSB(:,1,idx)],'datatestidx.csv')
end
